...
# for Document per CONST-035 + CONST-050(B). Cascade per CONST-051(A).

set -uo pipefail
HEALTH_URL="${DOCUMENT_HEALTH_URL:-}"
CHAOS_HOST="${DOCUMENT_CHAOS_HOST:-localhost}"
CHAOS_PORT="${DOCUMENT_CHAOS_PORT:-}"
LEGIT_REQS="${CHAOS_LEGIT_REQUESTS:-100}"
MIN_PCT="${CHAOS_LEGIT_MIN_PASS_PCT:-95}"

echo "=== Document Chaos Failure-Injection Challenge ==="
echo "  url=$HEALTH_URL host=${CHAOS_HOST}:${CHAOS_PORT}"

if [[ -z "$HEALTH_URL" ]] || [[ -z "$CHAOS_PORT" ]]; then
    echo "[1/6] SKIP: DOCUMENT_HEALTH_URL/_PORT unset — SKIP-OK: #env-no-target"
    echo "=== Document Chaos Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Document Chaos Challenge: PASSED ==="
